# Tasks: Navigation and project discovery
...
**Independent acceptance**: Given a work URL with search and filters, when the page loads or the visitor resets the controls, then the visible set, count, order, and URL agree.

- [x] T003 [US1] Map US1 requirements to named evidence in `plan.md` without claiming unperformed checks.

## Phase 4: US2

**Independent acceptance**: Given a deep link or chapter selection, when the reader scrolls, resizes, or selects another chapter before alignment finishes, then the latest valid intent determines the selected chapter.
