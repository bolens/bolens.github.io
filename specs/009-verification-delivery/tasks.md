# Tasks: Verification, delivery, and repository tooling
...
**Independent acceptance**: Given a proposed change, when lint and the full test command run, then invalid source or behavior fails with bounded diagnostics and no retry-until-green loop.

- [x] T003 [US1] Map US1 requirements to named evidence in `plan.md` without claiming unperformed checks.

## Phase 4: US2

**Independent acceptance**: Given an authorized reviewed PR with passing current-head checks, when it merges, then Pages may deploy only the exact still-current validated main revision and the public file allowlist.
