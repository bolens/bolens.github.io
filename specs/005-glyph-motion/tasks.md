# Tasks: Shared glyphs and page motion
...
**Independent acceptance**: Given a page row at a narrow or desktop width, when the visitor hovers or focuses its link, then text stays within its cell and the arrow/glyph remains separated from the label.

- [x] T003 [US1] Map US1 requirements to named evidence in `plan.md` without claiming unperformed checks.

## Phase 4: US2

**Independent acceptance**: Given an active illustration, when reduced motion or an overlay becomes active, then motion pauses or reduces without leaving incomplete artwork or restarting timelines on duplicate notifications.
