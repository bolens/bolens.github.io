...
| `assets/case-study.js` | `case-navigation.mjs` controls geometry and frames to check reading-line boundaries, coalesced scrolling, resize, deep links, rapid hashes, cancelled alignment, clicks, and empty/single-section documents. Browser command tests verify integration. |
...
| `assets/hobby-motion.js` | `hobby-motion-contract.mjs` checks visibility, overlay, system and saved motion gates, and overlapping pause reasons with controlled observer events. |
...
- Wait for actual completion state. A changed hash alone does not mean chapter
  scrolling has finished. Animation checks seek explicit phases. Command tests
  that depend on chapter alignment use reduced motion.
