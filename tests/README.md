...
| `assets/page-transitions.js` | `page-transition-contract.mjs` checks route and history direction, reduced-motion gates, fragments, and native fallbacks. `page-transitions.mjs` pauses explicit browser snapshots to check shared names, timing, single entrances, and responsive motion. Native links, Back, fragments, and reduced motion are checked separately because browsers may decline cross-document snapshots. |
