...
try {
  ...
  await send('Page.addScriptToEvaluateOnNewDocument', { source: `
    ...
    new MutationObserver((_, observer) => {
      ...
      window.__hybridReadyState = {
        painted: pixels.some((channel, index) => index % 4 === 3 && channel > 0),
  ...
  const landscapeMotion = await send('Runtime.evaluate', { expression: `new Promise((resolve)=>{const figure=document.querySelector('.cryptid-camp');const box=figure.getBoundingClientRect();figure.dispatchEvent(new PointerEvent('pointermove',{clientX:box.right-1,clientY:box.top+1,pointerType:'mouse'}));requestAnimationFrame(()=>{const style=getComputedStyle(figure);const offsets=['back','far','mid','near'].map((depth)=>Number.parseFloat(style.getPropertyValue('--parallax-'+depth+'-x')));document.documentElement.dataset.weather='rainy';const rain=document.querySelector('.weather-rain');const animation=rain.getAnimations()[0];animation.pause();animation.currentTime=0;const rainStart=getComputedStyle(rain).translate;const visible={rain:getComputedStyle(rain).display,clouds:getComputedStyle(document.querySelector('.weather-clouds')).display};animation.currentTime=550;const weather={...visible,rainStart,rainMiddle:getComputedStyle(rain).translate};delete document.documentElement.dataset.weather;resolve({offsets,weather})})})`, awaitPromise: true, returnByValue: true });
  ...
  await send('Runtime.evaluate', { expression: `document.querySelector('.cryptid-camp').dispatchEvent(new PointerEvent('pointerleave'))` });
  ...
  await send('Input.dispatchKeyEvent', { type: 'rawKeyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  const parallaxReset = await send('Runtime.evaluate', { expression: `new Promise((resolve)=>{const figure=document.querySelector('.cryptid-camp');const box=figure.getBoundingClientRect();figure.dispatchEvent(new PointerEvent('pointermove',{clientX:box.right-1,clientY:box.bottom-1,pointerType:'mouse'}));dispatchEvent(new CustomEvent('ui-overlay-change',{detail:{active:true}}));requestAnimationFrame(()=>{const style=getComputedStyle(figure);resolve(['back','far','mid','near'].flatMap((depth)=>[style.getPropertyValue('--parallax-'+depth+'-x'),style.getPropertyValue('--parallax-'+depth+'-y')]).map((value)=>Number.parseFloat(value)))})})`, awaitPromise: true, returnByValue: true });
  ...
  const hobbyOffscreen = await send('Runtime.evaluate', { expression: `(()=>{const section=document.querySelector('.hobbies');const traveler=document.querySelector('.hobby-traveler');return {state:section.dataset.motion,css:getComputedStyle(traveler).animationPlayState,svg:[...section.querySelectorAll('svg')].every((item)=>item.animationsPaused?.()??true)}})()`, returnByValue: true });
  if (hobbyOffscreen.result.value.state !== 'paused' || hobbyOffscreen.result.value.css !== 'paused' || !hobbyOffscreen.result.value.svg) throw new Error(`offscreen hobby motion failed: ${JSON.stringify(hobbyOffscreen.result.value)}`);
  ...
  const shortcutHelp = await send('Runtime.evaluate', { expression: `(()=>{const shortcuts=document.querySelector('.shortcut-overlay');const commands=document.querySelector('.command-palette');return {open:shortcuts.open,commandsOpen:commands.open,title:shortcuts.querySelector('h2')?.textContent,rows:shortcuts.querySelectorAll('tbody tr').length,overlay:portfolioOverlay.active}})()`, returnByValue: true });
  const shortcutHelpValue = shortcutHelp.result.value;
  if (!shortcutHelpValue.open || shortcutHelpValue.commandsOpen || shortcutHelpValue.title !== 'Keyboard shortcuts' || shortcutHelpValue.rows < 8 || !shortcutHelpValue.overlay) throw new Error(`shortcut help failed: ${JSON.stringify(shortcutHelpValue)}`);
  await send('Input.dispatchKeyEvent', { type: 'rawKeyDown', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  await send('Input.dispatchKeyEvent', { type: 'keyUp', key: 'Escape', code: 'Escape', windowsVirtualKeyCode: 27 });
  ...
  const skipLink = await send('Runtime.evaluate', { expression: `(()=>{const link=document.querySelector('.skip-link');const button=document.querySelector('.button');const style=getComputedStyle(link);const buttonStyle=getComputedStyle(button);const shine=getComputedStyle(link,'::after');const box=link.getBoundingClientRect();return {focused:document.activeElement===link,matchesFocus:link.matches(':focus-visible'),transform:style.transform,background:style.backgroundColor,color:style.color,border:style.borderColor,outline:style.outlineStyle,outlineWidth:style.outlineWidth,width:box.width,height:box.height,top:box.top,radius:style.borderRadius,padding:style.padding,overflow:style.overflow,isolation:style.isolation,buttonRadius:buttonStyle.borderRadius,buttonPadding:buttonStyle.padding,shine:shine.content}})()`, returnByValue: true });
  const skipValue = skipLink.result.value;
  if (!skipValue.focused || !skipValue.matchesFocus || skipValue.transform !== 'none' || skipValue.background === skipValue.color || skipValue.border !== skipValue.background || skipValue.outline === 'none' || Number.parseFloat(skipValue.outlineWidth) < 3 || skipValue.width < 24 || skipValue.height < 24 || skipValue.top < 0 || skipValue.radius !== skipValue.buttonRadius || skipValue.padding !== skipValue.buttonPadding || skipValue.overflow !== 'clip' || skipValue.isolation !== 'isolate' || skipValue.shine === 'none') throw new Error(`skip link focus failed: ${JSON.stringify(skipValue)}`);
  if (captureEvidence) {
    const skipCapture = await send('Page.captureScreenshot', { format: 'png', fromSurface: true });
    writeFileSync('/tmp/bolens-skip-link-focused.png', Buffer.from(skipCapture.data, 'base64'));
  ...
  const motion = await send('Runtime.evaluate', { expression: `(()=>{
    ...
    const liquid=document.querySelector('.beer-liquid');const liquidAnimation=liquid.getAnimations()[0];liquidAnimation.pause();liquidAnimation.currentTime=11500;const liquidBeforeFill=Number(getComputedStyle(liquid).opacity);liquidAnimation.currentTime=14000;const foam=document.querySelector('.beer-foam');const foamAnimation=foam.getAnimations()[0];const foamRiseAnimation=foam.getAnimations().find((item)=>item.animationName==='beer-foam-rise');const fillFoam=document.querySelector('.beer-fill-foam');const fillFoamRiseAnimation=fillFoam.getAnimations().find((item)=>item.animationName==='beer-foam-rise');for(const animation of [foamAnimation,foamRiseAnimation,fillFoamRiseAnimation])animation.pause();foamAnimation.currentTime=11500;const foamBeforeFill=Number(getComputedStyle(foam).opacity);foamAnimation.currentTime=14000;const foamDuringFill=Number(getComputedStyle(foam).opacity);const foamRise=[12480,14000,15600].map((time)=>{liquidAnimation.currentTime=time;foamRiseAnimation.currentTime=time;fillFoamRiseAnimation.currentTime=time;const liquidTop=liquid.getBoundingClientRect().top;const outlineTop=foam.getBoundingClientRect().top;const fillTop=fillFoam.getBoundingClientRect().top;return {time,liquidTop,outlineTop,fillTop,outlineGap:Math.abs(outlineTop-liquidTop),fillGap:Math.abs(fillTop-liquidTop)}});liquidAnimation.currentTime=20000;foamAnimation.currentTime=20000;const beerAfterDeparture={liquid:Number(getComputedStyle(liquid).opacity),foam:Number(getComputedStyle(foam).opacity)};liquidAnimation.currentTime=14000;foamAnimation.currentTime=14000;
    ...
    const flightPath=document.querySelector('#hobby-flight-path');const flightPathLength=flightPath.getTotalLength();const fixedInFlight=new DOMPoint(fixedDiscAtTap.x,fixedDiscAtTap.y).matrixTransform(hobby.getScreenCTM().inverse());const pathSamples=[.7479,.7499,.7519,.7539,.7559].map((progress)=>{const point=flightPath.getPointAtLength(flightPathLength*progress).matrixTransform(flightPath.getScreenCTM());return {targetX:fixedInFlight.x,targetY:fixedInFlight.y,progress,x:point.x,y:point.y,gap:Math.hypot(point.x-fixedDiscAtTap.x,point.y-fixedDiscAtTap.y)}});
  ...
  const sceneColorChecks = await send('Runtime.evaluate', { expression: `(()=>{const liquid=document.querySelector('.beer-liquid');const fillFoam=document.querySelector('.beer-fill-foam');const outlineFoam=document.querySelector('.beer-foam');const badgeTree=document.querySelector('.camp-tree');const routeTree=document.querySelector('.route-camp-tree-foreground');const animations=[liquid.getAnimations().find((item)=>item.animationName==='beer-liquid-color'),fillFoam.getAnimations().find((item)=>item.animationName==='beer-foam-color'),outlineFoam.getAnimations().find((item)=>item.animationName==='beer-foam-color'),badgeTree.getAnimations().find((item)=>item.animationName==='color-tree'),routeTree.getAnimations().find((item)=>item.animationName==='route-tree-color')];for(const animation of animations)animation.pause();const sample=(time)=>{for(const animation of animations)animation.currentTime=time;return {liquid:getComputedStyle(liquid).fill,fillFoam:getComputedStyle(fillFoam).fill,outlineFoamFill:getComputedStyle(outlineFoam).fill,outlineFoamStroke:getComputedStyle(outlineFoam).stroke,badgeTree:getComputedStyle(badgeTree).fill,routeTree:getComputedStyle(routeTree).fill}};return {campActive:sample(4320),beerActive:sample(14000),settled:sample(18000)}})()`, returnByValue: true });
  ...
  if (transparent(sceneColors.campActive.badgeTree) || transparent(sceneColors.campActive.routeTree) || transparent(sceneColors.beerActive.liquid) || transparent(sceneColors.beerActive.fillFoam) || transparent(sceneColors.beerActive.outlineFoamStroke) || !transparent(sceneColors.settled.liquid) || !transparent(sceneColors.settled.fillFoam) || !transparent(sceneColors.settled.outlineFoamFill) || !transparent(sceneColors.settled.outlineFoamStroke) || !transparent(sceneColors.settled.badgeTree) || !transparent(sceneColors.settled.routeTree)) throw new Error(`SVG scene color regression: ${JSON.stringify(sceneColors)}`);
